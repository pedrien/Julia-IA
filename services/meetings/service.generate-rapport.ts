...
/**
 * Génère un rapport pour une réunion en envoyant une requête GET à l'API.
 *
 * @function generateRapport
 * @description
 * Cette server action génère un rapport pour une réunion spécifiée.
 * Elle valide l'ID de la réunion, récupère la session utilisateur actuelle,
 * et envoie une requête GET à l'endpoint de génération de rapport de l'API.
 ...
 *
 * @example
 * const result = await generateRapport({ id: "0198e245-3fac-72a1-802d-5d3bfa164587" });
 * if (result.success) {
 ...
 * }
 */
export const generateRapport = actionClient
  .inputSchema(generateRapportSchema)
  .action(
    async ({ parsedInput: { id } }): Promise<IActionSuccess | IActionError> => {
      try {
        ...
        const response = await axios.get(
          `${ENV.API_LOCAL_BASE_URL}meetings/${id}/generate-rapport`,
          {
            headers: {
        ...
        if (response.status !== 200) {
          ...
          return {
            ...
            error: [
              errorResponse ||
                "Il semble y avoir une erreur lors de la génération du rapport.",
            ],
          };
        }
