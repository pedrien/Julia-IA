...
/**
 * Fetches a list of folders from the remote API.
 *
 * @function getListFolders
 * @returns {Promise<(IActionSuccess & { data: ListFolderSchema }) | IActionError>}
 *   An object indicating the success or failure of the operation, and the folders list data if successful.
 *
 ...
 * - On success, returns the list of folders.
 *
 * Example usage:
 *   const result = await getListFolders();
 *   if (result.success) {
 *     // Access result.data
 *   } else {
 *     // Handle result.error
 *   }
 */
export const getListFolders = actionClient.action(
  async (): Promise<
    (IActionSuccess & { data: ListFolderSchema }) | IActionError
  > => {
    try {
      ...
      const response = await axios.get(`${ENV.API_LOCAL_BASE_URL}folders`, {
        headers: {
      ...
      const responseData: ListFolderSchema = response.data.data;
